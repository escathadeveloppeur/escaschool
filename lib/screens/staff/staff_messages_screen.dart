// lib/screens/staff/staff_messages_screen.dart
...
class StaffMessagesScreen extends StatefulWidget {
  ...
  @override
  _StaffMessagesScreenState createState() => _StaffMessagesScreenState();
}
...
class _StaffMessagesScreenState extends State<StaffMessagesScreen>
  ...
  List<Map<String, dynamic>> otherStaffs = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // 🔑 CRUCIAL: Utiliser firestoreId comme dans parent
      final staffFirestoreId = auth.user?.firestoreId;
      final staffName = auth.user?.name ?? 'Personnel';
      
      print('\n╔════════════════════════════════════════════════════════════╗');
      print('║     CHARGEMENT STAFF MESSAGES                              ║');
      print('╚════════════════════════════════════════════════════════════╝\n');
      print('📌 STAFF FIRESTORE ID: $staffFirestoreId');
      print('📌 STAFF NAME: $staffName');
      
      // Charger les infos du staff
      staffInfo = {
        'id': staffFirestoreId,
        'name': staffName,
        'role': 'Staff',
      ...
      await _loadParents();
      await _loadAdmins();
      await _loadOtherStaffs(staffFirestoreId);
      await _loadConversations(staffFirestoreId);
  ...
  Future<void> _loadOtherStaffs(String? currentStaffId) async {
    print('\n🔍 CHARGEMENT DES AUTRES PERSONNELS');
    final snapshot = await FirebaseFirestore.instance
        .collection('staff')
        .get();
    ...
    print('   ✅ ${otherStaffs.length} autre(s) personnel(s)');
  }
  
  Future<void> _loadConversations(String? staffFirestoreId) async {
    if (staffFirestoreId == null) return;
    
    print('\n🔍 CHARGEMENT DES CONVERSATIONS');
    print('   🔑 Utilisation de staffFirestoreId: $staffFirestoreId');
    
    // 🔑 CRUCIAL: Utiliser recipientId = staffFirestoreId (comme parent)
    final receivedSnapshot = await FirebaseFirestore.instance
        .collection('messages')
        .where('recipientRole', isEqualTo: 'staff')
        .where('recipientId', isEqualTo: staffFirestoreId)
        .get();
    
    // 🔑 CRUCIAL: Utiliser senderId = staffFirestoreId (comme parent)
    final sentSnapshot = await FirebaseFirestore.instance
        .collection('messages')
        .where('senderRole', isEqualTo: 'staff')
        .where('senderId', isEqualTo: staffFirestoreId)
        .get();
  ...
  Future<void> _markMessagesAsRead(String contactId, String staffFirestoreId) async {
  ...
  Future<void> _sendMessage(String? staffFirestoreId, String staffName) async {
    final content = _contentController.text.trim();
    
    if (content.isEmpty || _selectedConversation == null || staffFirestoreId == null) {
    ...
    try {
      // 🔑 CRUCIAL: Utiliser les mêmes champs que parent
      final messageData = {
        'senderId': staffFirestoreId,
  ...
  void _showNewConversationDialog(String staffFirestoreId, String staffName) {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (teachers.isNotEmpty) ...[
                            ...
                            ...teachers.map((teacher) => _buildContactTile(
                              ...
                              onTap: () {
                                ...
                                _showConversationDetail(staffFirestoreId, staffName);
                              },
                            )).toList(),
                            const SizedBox(height: 16),
                          ],
                          ...
                          if (otherStaffs.isNotEmpty) ...[
                            ...
                            const SizedBox(height: 8),
                            ...otherStaffs.map((staff) => _buildContactTile(
                              name: staff['name'],
                              role: 'staff',
                              subtitle: staff['role'],
                              icon: Icons.person,
                          ...
                          if (teachers.isEmpty && students.isEmpty && parents.isEmpty && admins.isEmpty && otherStaffs.isEmpty)
                            Padding(
  ...
  void _showConversationDetail(String staffFirestoreId, String staffName) async {
    if (_selectedConversation == null) return;
    
    await _markMessagesAsRead(_selectedConversation!['contactId'], staffFirestoreId);
                      ...
                      CircleAvatar(
                        ...
                        child: IconButton(
                          ...
                          onPressed: _isSending ? null : () async {
                            await _sendMessage(staffFirestoreId, staffName);
                            setStateBottomSheet(() {});
                            await _loadConversations(staffFirestoreId);
                          },
                        ),
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => _showNewConversationDialog(staffFirestoreId!, staffName),
          ),
        ],
      ),
      body: _isLoading
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(
                        ...
                        child: const Icon(Icons.person, size: 20, color: Colors.purple),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(staffName, style: const TextStyle(fontWeight: FontWeight.w600)),
                            Text('Personnel', style: TextStyle(fontSize: 12, color: Colors.grey[500])),
                          ],
                        ),
                      ),
                    ],
